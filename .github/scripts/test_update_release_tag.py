...
set -euo pipefail
printf '%s\n' "$*" >> "$FAKE_GH_LOG"
method=GET; endpoint="$2"
if [[ "$2" == --method ]]; then method="$3"; endpoint="$4"; fi
reads=0
[[ ! -f "$FAKE_GH_STATE" ]] || reads="$(cat "$FAKE_GH_STATE")"
if [[ "$SCENARIO" == read-error && "$method" == GET ]]; then echo 'gh: Forbidden (HTTP 403)' >&2; exit 1; fi
if [[ "$method" == GET ]]; then
  reads=$((reads + 1)); echo "$reads" > "$FAKE_GH_STATE"
  if [[ "$SCENARIO" == missing && "$reads" -eq 1 ]]; then echo 'gh: Not Found (HTTP 404)' >&2; exit 1; fi
  result_sha="$GITHUB_SHA"
  [[ "$SCENARIO" != mismatch || "$reads" -le 1 ]] || result_sha="bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
  printf '{"object":{"type":"commit","sha":"%s"}}\n' "$result_sha"; exit 0
fi
if [[ "$method" == PATCH ]]; then [[ "$endpoint" == repos/bywinsty/cs2-vip/git/refs/tags/dev-modules ]] || exit 2; echo '{}'; exit 0; fi
if [[ "$method" == POST ]]; then [[ "$endpoint" == repos/bywinsty/cs2-vip/git/refs ]] || exit 2; echo '{}'; exit 0; fi
exit 2
'''
...
class UpdateReleaseTagTests(unittest.TestCase):
    def run_helper(self, scenario: str):
        with tempfile.TemporaryDirectory(dir=SCRIPT.parents[2]) as directory:
            ...
            fake_gh.chmod(0o755)
            environment = {
                "GH_BIN": f"./{root.name}/gh", "SCENARIO": scenario,
                "FAKE_GH_STATE": f"./{root.name}/state", "FAKE_GH_LOG": f"./{root.name}/calls.log",
                "GITHUB_REPOSITORY": "bywinsty/cs2-vip", "GITHUB_REF_NAME": "dev-modules",
                "GITHUB_SHA": SHA, "GITHUB_RUN_ID": "123",
                "GITHUB_STEP_SUMMARY": f"./{root.name}/summary.md", "RELEASE_TAG": "dev-modules",
            }
            ...
            result = subprocess.run(
                ...
                env=os.environ, cwd=SCRIPT.parents[2], check=False,
            )
    ...
    def test_existing_ref_uses_the_canonical_patch_endpoint(self):
        result, calls, summary = self.run_helper("existing")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn(f"api --method PATCH repos/bywinsty/cs2-vip/git/refs/tags/dev-modules -f sha={SHA} -F force=true", calls)
        self.assertNotIn("git/ref/tags/dev-modules", "\n".join(call for call in calls if "--method" in call))
        self.assertIn("type=commit", summary)

    def test_missing_ref_creates_the_branch_tag(self):
        result, calls, _ = self.run_helper("missing")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn(f"api --method POST repos/bywinsty/cs2-vip/git/refs -f ref=refs/tags/dev-modules -f sha={SHA}", calls)

    def test_non_404_and_wrong_final_sha_fail_with_diagnostics(self):
