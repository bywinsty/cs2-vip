...
def main() -> int:
    ...
    parser.add_argument("--check-ambuild", action="store_true")
    args = parser.parse_args()
    if sys.version_info[:2] != EXPECTED_PYTHON:
        raise SystemExit(f"Expected Python 3.12, got {sys.version_info.major}.{sys.version_info.minor}")
    import setuptools
    if setuptools.__version__ != EXPECTED_SETUPTOOLS:
        raise SystemExit(f"Expected setuptools {EXPECTED_SETUPTOOLS}, got {setuptools.__version__}")
